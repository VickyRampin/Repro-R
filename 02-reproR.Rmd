...
download.file("https://gitlab.com/investigating-archiving-git/survey-analysis/-/raw/master/00-prep-data.Rmd?inline=false", "src/processing.Rmd", mode = "wb")
...
Let's snapshot that, because probably a lot of packages were installed just then between `ggplot2` here and what's in our processing script:

```{r snapshot01}
renv::snapshot()
```

The following figures were generated as a part of a submission to HICSS 2020 conference. We're going to try to run this code, and
...
```
And we'll snapshot at the end as well to make sure any last dependencies are picked up by `renv`: 
